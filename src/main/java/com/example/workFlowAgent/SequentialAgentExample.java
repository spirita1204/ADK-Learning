...
public class SequentialAgentExample {
  ...
  public void runAgent(String prompt) {
    LlmCofig llmCofig = new LlmCofig();
    LlmAgent codeWriterAgent = LlmAgent.builder().model(llmCofig.createModel()).name("CodeWriterAgent")
        .description("Writes initial Java code based on a specification.").instruction("""
            ...
            """).outputKey("generated_code").build();

    LlmAgent codeReviewerAgent = LlmAgent.builder().model(llmCofig.createModel()).name("CodeReviewerAgent")
        .description("Reviews code and provides feedback.").instruction("""
            ...
            """).outputKey("review_comments").build();

    LlmAgent codeRefactorerAgent = LlmAgent.builder().model(llmCofig.createModel()).name("CodeRefactorerAgent")
        .description("Refactors code based on review comments.").instruction("""
            ...
            **Output:**
            ...
            Do not add any other text before or after the code block.
            """).outputKey("refactored_code").build();

    SequentialAgent codePipelineAgent = SequentialAgent.builder().name(APP_NAME)
        .description("Executes a sequence of code writing, reviewing, and refactoring.")
        // The agents will run in the order provided: Writer -> Reviewer -> Refactorer
        .subAgents(codeWriterAgent, codeReviewerAgent, codeRefactorerAgent).build();
